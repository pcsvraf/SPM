...
$columns = array("facturasObraRel.id", "facturasObraRel.numeroFactura", "facturasObraRel.fecha",
    "facturasObraRel.detalleServicio", "facturasObraRel.totalFactura", "facturasObraRel.devolucionDeRetencion");

$query = "SELECT * FROM facturasObraRel";

$query .= " WHERE idRelacion='$id' &&";

if (isset($_POST["search"]["value"])) {
    $query .= '(facturasObraRel.id LIKE "%' . $_POST["search"]["value"] . '%" ';
    $query .= 'OR facturasObraRel.numeroFactura LIKE "%' . $_POST["search"]["value"] . '%" ';
    $query .= 'OR facturasObraRel.fecha LIKE "%' . $_POST["search"]["value"] . '%" ';
    $query .= 'OR facturasObraRel.detalleServicio LIKE "%' . $_POST["search"]["value"] . '%" ';
    $query .= 'OR facturasObraRel.devolucionDeRetencion LIKE "%' . $_POST["search"]["value"] . '%" ';
    $query .= 'OR facturasObraRel.totalFactura LIKE "%' . $_POST["search"]["value"] . '%") ';
}
...
while ($row = mysqli_fetch_array($result)) {
    $consulta1 = "SELECT nombreArchivo FROM archivosFacturasObraRel WHERE idRelacion=" . $row["id"];
    ...
    //$sub_array[] = '<input type="text" contenteditable class="update form-control" data-id="' . $row["id"] . '" data-column="detalleServicio" value="' . $row["detalleServicio"] . '" disabled>';
